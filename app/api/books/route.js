...
export async function POST(request) {
  ...
  if (Array.isArray(requestBody)) {
    ...
    try {
      await db.collection("books").insertMany(requestBody);
    } catch (error) {
      console.log(error);
      client.close();
      return new Response("Multiple book creation is failed", {
        status: 500,
        headers: { referer: referer },
      });
    }
  ...
  try {
    ...
    const result = await db.collection("books").insertOne(requestBody);
  } catch (error) {
    console.log(error);
    client.close();
    return new Response("single book creation failed!", {
      status: 500,
      headers: { referer: referer },
    });
  }
